	...
	//Brunel University, Department of Computer Science, Module CS1702, 2016-2017
	//Dr Stephen Swift's Rather Crude Finch Testing Program
	//Hopefully this works...
	//This is for week zero/fresher's week/induction week to test to see if the 
	//Finch Robot works
	...
	public class DoesMyFinchWork 
	{
		...
		public static void main(String args[]) 
		{
			...
			do
			{
				...
				s = FinchMenu();
				if (s.equals("Light Test")) RunLightTest(s);
				if (s.equals("Tilt Test")) RunTiltTest(s);
				if (s.equals("Tap Test")) RunTapTest(s);
				if (s.equals("Temperature Test")) RunTemperatureTest(s);
				if (s.equals("Obstacle Test")) RunObstacleTest(s);
				if (s.equals("Acceleration Test")) RunAccelerationTest(s);
				if (s.equals("Left Wheel Test")) RunLeftWheelTest(s);
				if (s.equals("Right Wheel Test")) RunRightWheelTest(s);
				if (s.equals("Buzzer Test")) RunBuzzerTest(s);
				if (s.equals("Nose Test")) RunNoseTest(s);
			} while (!(s.equals("Quit"))); 
		...
		myfinch.setLED(255, 255, 255, 10000);
		myfinch.quit();
	}	
